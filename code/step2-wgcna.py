...
def module_trait_correlation(ME_df, labels, method='permutation', one_tailed=True, n_permutations=10000):
    """
    Calculate module-trait correlations with FDR correction.
    
    Args:
        ME_df: Module eigengene dataframe
        labels: Sample labels (case/control)
        method: Correlation method ('pearson', 'spearman', 'bicor', or 'permutation')
        one_tailed: Use one-tailed test (default: True, for positive correlations)
        n_permutations: Number of permutations for permutation test
    
    Returns:
        cor_df: DataFrame with correlation results
    """
    from statsmodels.stats.multitest import multipletests

    print(f"  Calculating module-trait correlations (method={method}, one_tailed={one_tailed})...")
    trait_numeric = np.array([0 if str(l).lower() == "control" else 1 for l in labels])

    correlations = []
    for i, col in enumerate(ME_df.columns):
        print(f"    Processing {col}... ({i+1}/{len(ME_df.columns)})", end='\r')
        
        if method == 'permutation':
            r, p = permutation_correlation_test(
                ME_df[col].values, trait_numeric, 
                n_permutations=n_permutations, 
                one_tailed=one_tailed
            )
        else:
            if method == 'pearson':
                r, p_2tail = stats.pearsonr(ME_df[col].values, trait_numeric)
            elif method == 'spearman':
                r, p_2tail = stats.spearmanr(ME_df[col].values, trait_numeric)
            elif method == 'bicor':
                r, p_2tail = bicor_with_pvalue(ME_df[col].values, trait_numeric)
            else:
    ...
    print(f"  Best module: {cor_df.iloc[0]['module']} (r={cor_df.iloc[0]['correlation']:.3f}, p={cor_df.iloc[0]['pvalue']:.4f}, q={cor_df.iloc[0]['qvalue']:.4f})")
    return cor_df


def permutation_correlation_test(x, y, n_permutations=10000, one_tailed=True):
    """
    ...
    Args:
        ...
        one_tailed: Use one-tailed test (for positive correlation)
    ...
    if one_tailed:
        if r_obs > 0:
            p_value = np.sum(r_perm >= r_obs) / n_permutations
        else:
            p_value = np.sum(r_perm <= r_obs) / n_permutations
    else:
        p_value = np.sum(np.abs(r_perm) >= np.abs(r_obs)) / n_permutations
    
    return r_obs, p_value


def bicor_with_pvalue(x, y):
    """
    Calculate biweight midcorrelation with p-value.
    
    This is the WGCNA standard correlation method, more robust to outliers
    ...
    # Calculate u values (standardized deviations)
    u_x = (x - med_x) / (9 * mad_x)
    u_y = (y - med_y) / (9 * mad_y)
    
    # Calculate weights (biweight function)
    ...
    # Calculate bicor
    numerator = np.sum(a_x * a_y)
    denominator = np.sqrt(np.sum(a_x**2) * np.sum(a_y**2))
    
    if denominator == 0:
        return 0, 1
    
    bicor = numerator / denominator
    
    # Calculate p-value using Fisher's z-transformation
    # Effective sample size (accounting for weights)
    n_eff = np.sum(w_x * w_y)
    
    if n_eff < 4:
        return bicor, 1
    
    # Fisher's z-transformation
    z = 0.5 * np.log((1 + bicor) / (1 - bicor + 1e-10))
    se = 1 / np.sqrt(n_eff - 3)
    z_score = z / se
    
    # Two-tailed p-value
    p_value = 2 * (1 - t_dist.cdf(np.abs(z_score), df=n_eff - 2))
    
    return bicor, p_value
...
if __name__ == "__main__":
    ...
    # Module-trait correlation using permutation test (one-tailed)
    print("\nMODULE-TRAIT ANALYSIS")
    cor_df = module_trait_correlation(ME_df, labels, method='permutation', one_tailed=True, n_permutations=10000)
    cor_df.to_csv(
