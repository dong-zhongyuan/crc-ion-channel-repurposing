...
PROJECT_ROOT = os.path.dirname(os.path.dirname(_PROJECT_DIR))
OUTPUT_DIR = f"{PROJECT_ROOT}/step1_deg_analysis/result"
DATA_FILE = f"{PROJECT_ROOT}/data/data.csv"
...
GENCODE_COORDS_FILE = (
    f"{PROJECT_ROOT}/data/externalvalidatedata/hg38_gene_symbol_lookup.tsv"
)
...
def get_metadata(df, labels=None):
    ...
    if labels is not None:
        # 使用标签行
        for label in labels:
            if "young" in str(label).lower():
                groups.append("Young")
            elif "old" in str(label).lower():
                groups.append("Old")
            else:
                groups.append(None)
    else:
        ...
        for sample in samples:
            ...
            if "young" in sample_lower or sample_lower.startswith("y"):
                groups.append("Young")
            elif "old" in sample_lower or sample_lower.startswith("o"):
                groups.append("Old")
            else:
                groups.append(None)
    ...
    logger.info(
        f"Young: {sum(g == 'Young' for g in groups)}, Old: {sum(g == 'Old' for g in groups)}"
    )
...
def perform_deg_analysis(df, metadata):
    ...
    young_samples = metadata[metadata["Group"] == "Young"]["SampleID"].tolist()
    old_samples = metadata[metadata["Group"] == "Old"]["SampleID"].tolist()

    logger.info(f"Young samples: {len(young_samples)}, Old samples: {len(old_samples)}")
    ...
    logger.info(f"  Upregulated (Old vs Young): {n_up}")
...
def plot_panel_A(df, metadata):
    ...
    colors = {"Young": NPG_COLORS["blue"], "Old": NPG_COLORS["red"]}
...
def plot_panel_F(df, deg_df, metadata):
    ...
    # 图例
    young_patch = mpatches.Patch(color=NPG_COLORS["blue"], label="Young")
    old_patch = mpatches.Patch(color=NPG_COLORS["red"], label="Old")
    ax.legend(
...
def plot_panel_G(deg_df, gene_coords, output_dir):
    ...
    # Normalize to chr1, chr2, ..., chrX, chrY format
    if "chrom" not in plot_df.columns and "chr" in plot_df.columns:
        # Rename 'chr' to 'chrom'
        plot_df = plot_df.rename(columns={"chr": "chrom"})
    elif "chrom" in plot_df.columns:
        # If already has chr prefix, keep it; otherwise add it
...
def plot_panel_J(expr_df, metadata, output_dir, deg_df=None):
    ...
    # Create violin plot
    palette = {"Young": NPG_COLORS["blue"], "Old": NPG_COLORS["red"]}
...
def plot_panel_M(expr_df, metadata, output_dir):
    ...
    group_colors = metadata.set_index("SampleID")["Group"].map(
        {"Young": NPG_COLORS["blue"], "Old": NPG_COLORS["red"]}
    )
...
def plot_panel_N(expr_df, metadata, output_dir):
    ...
    dend = dendrogram(
        linkage_matrix,
        labels=expr_matrix.index.tolist(),
        leaf_rotation=90,
        leaf_font_size=8,
        ax=ax,
        color_threshold=0,
    ...
    ax.set_ylabel("Distance (Ward)", fontsize=10)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    legend_elements = [
        Patch(facecolor=NPG_COLORS["blue"], label="Young"),
        Patch(facecolor=NPG_COLORS["red"], label="Old"),
    ]
...
def plot_panel_P(metadata, output_dir):
    ...
    # Count samples per group (Group column with Young/Old values)
    young_n = len(metadata[metadata["Group"] == "Young"])
    old_n = len(metadata[metadata["Group"] == "Old"])
...
def generate_audit_files(output_dir, deg_df, gene_coords, metadata, lambda_gc):
    ...
    manifest = {
        ...
        "results_summary": {
            ...
            "lambda_gc": lambda_gc,
            "n_samples_young": len(metadata[metadata["Group"] == "Young"]),
            "n_samples_old": len(metadata[metadata["Group"] == "Old"]),
        },
    ...
    summary_lines = [
        ...
        "",
        "SAMPLES:",
        f"  Young: {manifest['results_summary']['n_samples_young']}",
        f"  Old: {manifest['results_summary']['n_samples_old']}",
        "",
        "OUTPUT FILES:",
    ]
...
def main():
    ...
    print(f"  Young: {len(metadata[metadata['Group'] == 'Young'])} samples")
    print(f"  Old: {len(metadata[metadata['Group'] == 'Old'])} samples")
