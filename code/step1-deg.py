...
# DEG 阈值
FDR_THRESHOLD = 0.05
LOG2FC_THRESHOLD = np.log2(1.5)  # ~0.585 (updated from log2(1.2) ≈ 0.263)
...
def perform_deg_analysis(df, metadata):
    ...
    control_samples = metadata[metadata["Group"] == "control"]["SampleID"].tolist()
    case_samples = metadata[metadata["Group"] == "case"]["SampleID"].tolist()

    logger.info(
        f"Control samples: {len(control_samples)}, Case samples: {len(case_samples)}"
    )

    results = []
    for gene in df.index:
        control_expr = df.loc[gene, control_samples].values
        case_expr = df.loc[gene, case_samples].values

        mean_control = np.mean(control_expr)
        mean_case = np.mean(case_expr)

        # Log2FC = Mean_case - Mean_control (数据已是 log 值)
        log2fc = mean_case - mean_control

        # Welch t-test
        t_stat, pvalue = stats.ttest_ind(case_expr, control_expr, equal_var=False)

        # 标准误 (用于置信区间)
        se_control = stats.sem(control_expr)
        se_case = stats.sem(case_expr)
        se_diff = np.sqrt(se_control**2 + se_case**2)

        # Cohen's d effect size (pooled standard deviation)
        n1, n2 = len(control_expr), len(case_expr)
        var1, var2 = np.var(control_expr, ddof=1), np.var(case_expr, ddof=1)
...
def plot_panel_D(deg_df):
    ...
    deg_df["AvgExpr"] = (deg_df["Mean_control"] + deg_df["Mean_case"]) / 2
...
def plot_panel_F(df, deg_df, metadata):
    ...
    control_samples = metadata[metadata["Group"] == "control"]["SampleID"].tolist()
    case_samples = metadata[metadata["Group"] == "case"]["SampleID"].tolist()
    ordered_samples = control_samples + case_samples
    ...
    # 创建颜色条
    group_colors = [NPG_COLORS["blue"]] * len(control_samples) + [
        NPG_COLORS["red"]
    ] * len(case_samples)
    ...
    # 图例
    control_patch = mpatches.Patch(color=NPG_COLORS["blue"], label="control")
    case_patch = mpatches.Patch(color=NPG_COLORS["red"], label="case")
    ax.legend(
        handles=[control_patch, case_patch],
        loc="upper left",
...
def plot_panel_P(metadata, output_dir):
    ...
    control_n = len(metadata[metadata["Group"] == "control"])
    case_n = len(metadata[metadata["Group"] == "case"])
    ...
    ax.text(2.25, 3.2, f"n = {control_n}", fontsize=11, ha="center", va="center")
    ax.text(2.25, 2.5, "samples", fontsize=10, ha="center", va="center")

    # Case group box
    case_box = plt.Rectangle(
        (6, 2),
        3.5,
        2.5,
        fill=True,
    ...
    ax.text(7.75, 3.2, f"n = {case_n}", fontsize=11, ha="center", va="center")
...
def generate_audit_files(output_dir, deg_df, gene_coords, metadata, lambda_gc):
    ...
    manifest = {
        ...
        "results_summary": {
            ...
            "lambda_gc": lambda_gc,
            "n_samples_control": len(metadata[metadata["Group"] == "control"]),
            "n_samples_case": len(metadata[metadata["Group"] == "case"]),
        },
    ...
    summary_lines = [
        ...
        "",
        "SAMPLES:",
        f"  Control: {manifest['results_summary']['n_samples_control']}",
        f"  Case: {manifest['results_summary']['n_samples_case']}",
        "",
        "OUTPUT FILES:",
    ]
