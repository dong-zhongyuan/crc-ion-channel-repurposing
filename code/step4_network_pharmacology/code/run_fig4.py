    ...
    if args.config:
        ...
        if config_path.exists():
            with open(config_path, "r") as f:
                ...
                _cfg_dir = config_path.parent
                # Update paths (resolve relative to config file location)
                if "hub_genes_csv" in file_config:
                    p = Path(file_config["hub_genes_csv"])
                    args.hub_genes = str(_cfg_dir / p if not p.is_absolute() else p)
                if "drug_mining_csv" in file_config:
                    p = Path(file_config["drug_mining_csv"])
                    args.drug_mining = str(_cfg_dir / p if not p.is_absolute() else p)
                if "output_dir" in file_config:
                    p = Path(file_config["output_dir"])
                    args.output = str(_cfg_dir / p if not p.is_absolute() else p)
                # Update parameters
                for key in [
                    "universe_size",
