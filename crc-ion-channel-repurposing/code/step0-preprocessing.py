...
DISCOVERY_DATASETS = {
    "GSE196006": {
        ...
        "id_type": "ensembl",
        "group_logic": "suffix",
        "tumor_pattern": "_L0_|_B0_|_E0_|_A0_|_D0_|_H0_",
        "normal_pattern": "_L7_|_B7_|_E7_|_A7_|_D7_|_H7_",
    },
    "GSE251845": {
...
def save_expression_csv(df, labels, sample_ids, output_path, dataset_labels=None):
    lines = ["sample," + ",".join(sample_ids), "label," + ",".join(labels)]
    if dataset_labels is not None:
        lines.append("dataset," + ",".join(dataset_labels))
...
def main():
    ...
    save_expression_csv(
        val_norm, val_labels, val_samples, OUTPUT_DIR / "external_validation_data.csv",
        dataset_labels=val_ds,
    )
    save_metadata_csv(
