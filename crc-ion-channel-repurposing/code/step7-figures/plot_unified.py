...
def make_figure_2(save_path=None):
    ...
    fc_thr = np.log2(1.5)  # dashed guide lines
    p_thr = -np.log10(0.05)
    # Color by source SignificantFlag (Up/Down/NS)
    _flag_color = {'Up': CASE, 'Down': BLUE, 'NS': LGRAY}
    vdf['_color'] = vdf['SignificantFlag'].map(_flag_color).fillna(LGRAY)
    for flag, color, sz in [('NS', LGRAY, 4), ('Up', CASE, 6), ('Down', BLUE, 6)]:
        m = vdf['SignificantFlag'] == flag
        ax.scatter(vdf.loc[m, 'Log2FC'], vdf.loc[m, '-log10p'],
                   c=color, s=sz, alpha=0.4 if flag == 'NS' else 0.45,
                   edgecolors='none', zorder=1, rasterized=True)
    ax.axhline(p_thr, ls='--', lw=0.6, color=MGRAY, zorder=1)
    ax.axvline(fc_thr, ls='--', lw=0.6, color=MGRAY, zorder=1)
    ax.axvline(-fc_thr, ls='--', lw=0.6, color=MGRAY, zorder=1)
    hub_mask = vdf['Gene'].isin(HUB20)
    hub_df = vdf[hub_mask].copy()
    hub_df['color'] = hub_df['SignificantFlag'].map(_flag_color).fillna(LGRAY)
    ...
    handles_vol = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor=CASE,
               markersize=5, markeredgecolor='none', label='Up-regulated'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=BLUE,
               markersize=5, markeredgecolor='none', label='Down-regulated'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=LGRAY,
               markersize=5, markeredgecolor='none', label='Not significant'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=MGRAY,
               markersize=8, markeredgecolor='black', markeredgewidth=0.5,
               label='Hub gene'),
    ]
    ax.legend(handles=handles_vol, loc='upper left', fontsize=7.5,
             title_fontsize=7.5)
